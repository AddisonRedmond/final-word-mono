...
const Duels = () => {
  ...
  const { data: duels, refetch: refetchDuels } = api.duels.allDuels.useQuery();
  ...
  const handleSendDuel = async (invitedFriends: Friend[]) => {
    sendDuelMutation.mutateAsync(invitedFriends.map((friend) => friend.id));
    refetchDuels();
  };
