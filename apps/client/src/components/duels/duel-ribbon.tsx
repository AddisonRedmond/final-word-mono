...
const DuelRibbon: React.FC<DuelRibbonProps> = ({
	...
	return (
		<div className="flex items-center justify-between rounded-md px-3 py-2.5 transition-colors hover:bg-stone-500/10">
			<div className="min-w-0">
				<div className="flex items-center gap-1.5">
					...
					{opponentIds.map((id) => {
						...
						return (
							<span
								className={`grid size-6 shrink-0 select-none place-content-center rounded-md font-bold text-[10px] text-white ${opponentColor(participant)}`}
								key={id}
								title={name}
							>
								{getInitials(name)}
			...
			{duel.completed ? (
				<div className="flex items-center gap-1.5">
					<span className="shrink-0 rounded-md bg-stone-200 px-2 py-1 font-bold text-[11px] text-stone-600 uppercase tracking-widest">
						Completed
					</span>
					{hasCompleted ? (
