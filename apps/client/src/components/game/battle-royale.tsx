  ...
  useWebSocket({
    ...
    onMessage: (msg: ClientMessage) => {
      if (msg.type === "lobby_state") {
        ...
        gameStore.setGameStarted(msg.lobbyId, myUserId ?? "", msg.players);
        setPhase("playing");
      ...
      } else if (msg.type === "player_health_update") {
        gameStore.applyHealthUpdate(msg);
      } else if (msg.type === "attack_word") {
        ...
        <div className="flex flex-col grow items-center justify-center gap-6">
          {status !== "open" ? (
            ...
            <>
              ...
              {countdownDigits ? (
                <ScrollTile letters={countdownDigits} theme="present" size="lg" />
              ) : (
      ...
      <div className="flex h-screen">
        ...
        <div className="flex flex-col gap-4 p-4 w-64 border-l border-white/10">
          ...
          <OpponentPanel
            ...
            onSelectTarget={(userId) => gameStore.setTarget(userId, sendMessage)}
          />
        </div>
      </div>
    </div>
