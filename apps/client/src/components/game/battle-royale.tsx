...
export default function BattleRoyale({onLeave} : BattleRoyaleProps) {
        ...
        onMessage: (msg : ClientMessage) => {
            switch (msg.type) {
                case "lobby_state":
                    ...
                    break;
        case "game_started":
                    useGameStore.getState().setGameStarted(msg.lobbyId, useAuthStore.getState().user?.uid ?? "", msg.players);
                    setPhase("playing");
                    break;
                case "guess_result": gameStore.applyGuessResult(msg);
                    break;
                case "game_state_update": gameStore.applyGameStateUpdate(msg);
                    break;
                case "player_health_update": gameStore.applyHealthUpdate(msg);
                    break;
                case "attack_word": gameStore.applyAttackWord(msg);
                    break;
                case "game_over": gameStore.applyGameOver(msg);
                    break;
            }
        }
    });
    ...
    useEffect(() => {
        ...
        const tick = () => {
            const secs = Math.max(0, Math.ceil((lobby.beginAtCountdown - Date.now()) / 1000));
            setCountdown(secs);
        };
        tick();
    ...
    const header = (
        <div className="flex items-center justify-between px-6 py-4 border-b border-white/10">
            <h1 className="font-mono text-lg uppercase tracking-widest">Battle Royale</h1>
        ...
        <div className="flex flex-col h-screen">
            ...
            <div className="flex h-screen">
                ...
                <div className="flex flex-col gap-4 p-4 w-64 border-l border-white/10">
                    <TargetSelector targetMode={targetMode}
                        onModeChange={
                            (mode) => useGameStore.setState({targetMode: mode})
                        }/>
                    <OpponentPanel opponents={opponents}
                        targetMode={targetMode}
                        selectedTargetId={targetUserId}
                        onSelectTarget={
                            (userId) => gameStore.setTarget(userId, sendMessage)
                        }/>
                </div>
            </div>
        </div>
