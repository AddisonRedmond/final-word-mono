 ...
 * --------------------
 * This file is the schema entrypoint that `drizzle.config.ts` points at, so it
 * defines exactly the tables Drizzle owns and generates migrations for:
 * `profiles`, `friendships`, and `game_player_stats`.
 *
 * The Supabase-owned duel tables live in `schema.duels.ts` and are intentionally
 * NOT imported here, so `drizzle-kit generate` / `push` / `pull` never see them
 * and cannot emit a colliding CREATE migration. See `schema.duels.ts` for the
 * ownership rationale.
 */
